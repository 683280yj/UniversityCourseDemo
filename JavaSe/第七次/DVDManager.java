...
//	private List dvdList = new ArrayList();	//DVD列表
	private Map dvdList = new HashMap();	//DVD列表
...
//		dvdList.add(dvd);
	...
	public void showAll(){
		...
		//遍历键，再根据键得到值
		for (Object obj : dvdList.keySet()) {
			DVD dvd = (DVD)dvdList.get(obj);	//根据key得到value
...
//		}
	...
	public int remove(String dvdNO){
		...
		if(dvdList.containsKey(dvdNO)){
			if(((DVD)dvdList.get(dvdNO)).getState()==0){
