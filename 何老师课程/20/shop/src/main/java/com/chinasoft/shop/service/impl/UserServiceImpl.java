...
//                session.setAttribute("msg","密码错误");
    ...
    @Override
    public int insertUserShopCart(Map<String, Object> map,HttpServletRequest request) {
        Map<String,Object> user=(Map<String, Object>) request.getSession().getAttribute("user");
        String userId=user.get("userId").toString();
        //取到session里面的userId
        map.put("userId",userId);
        List<Map<String,Object>> list=userMapper.queryUserShopCartList(map);//map中一个参数就是产品的Id
        //现实对应的用户购物车下的所有商品
        if(list.size()==0){
            //购物车为空
            ...
            map.put("userShopCarProductId",map.get("productId").toString());
//            Map<String,Object> productMap=userMapper.queryUserShopCartListByUserIdAndProductId(map);
            //userId 和 userShopCartProductId在上面的map中都已存在
        ...
        return userMapper.insertUserShopCart(map);
    }
}
