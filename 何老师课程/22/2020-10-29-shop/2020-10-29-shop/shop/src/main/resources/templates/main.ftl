...
<script>
	$(function(){
		...
		var s=['am-u-sm-7 am-u-md-4 text-two sug','am-u-sm-7 am-u-md-4 text-two','am-u-sm-3 am-u-md-2 text-three big','am-u-sm-3 am-u-md-2 text-three sug','am-u-sm-3 am-u-md-2 text-three','am-u-sm-3 am-u-md-2 text-three last big'];
		for(var i=0;i<count;i++){
			$("#div_"+i).attr('class',s[i]);
		}
	})

	//添加商品加入到购物车的时候这个用户必须是登录的情况下才可以操作
	//在js里面是不能够得到对象来进行判断的，必须得对象.属性的形式去判断
	function addProduct(productId){
if('${user.userEmail}'==''){
	//alert('没有经过登录');
	location.href='login';//正确来说应该是进入某个方法里面传递一个session.setAtture一个msg消息没有经过登录跳到登录页面
}else{
	//alert('可以执行添加购物车的操作');
	//那么这里必须要用ajax来做，因为添加成功之后那么在main.ftl上面必须要有购物车的数量的刷新,ajax
	//因为是实现无刷新效果，所以用ajax做会比较好
	$.ajax({
		url:'addProduct',
		dataType:'json',
		type:'post',
		...
		success:function(data){
			...
			//console.log(data);
			alert(data.msg);
			//window.location.hash = "#J_MiniCartNum";//进入锚点到购物车的那个位置
			$("#J_MiniCartNum").html(data.count);
			window.scrollTo(0,0);
			//window.history.back();
		}
			...
			<div class="am-container header">
				...
				<ul class="message-r">
					...
					<div class="topMessage my-shangcheng">
						<div class="menu-hd MyShangcheng"><a href="peoplecenter" target="_top"><i class="am-icon-user am-icon-fw"></i>个人中心</a></div>
					</div>
					<div class="topMessage mini-cart">
						<div class="menu-hd"><a id="mc-menu-hd" href="myShopCart" target="_top"><i class="am-icon-shopping-cart  am-icon-fw"></i><span>购物车</span><strong id="J_MiniCartNum" class="h">${userShopCartCount}</strong></a></div>
					</div>
						...
						<#list list as item>
							<div id="div_${item_index}">
								<div class="outer-con ">
									...
									<a href="javascript:addProduct(${item.productId});"><i class="am-icon-shopping-basket am-icon-md  seprate"></i></a>
								</div>
								<a href="#"><img src="${item.productPhotoUrl}" /></a>
							</div>
						</#list>
